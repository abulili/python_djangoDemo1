...
# logger.debug(create_log('aaa', 0.5,'04'))

# # 异常处理  不清楚什么异常可以先写 except Exception as e，把具体异常类型打印出来 logger.debug(f"异常类型：{type(e).__name__}，信息：{e}")，然后改成对应的
...
# logger.debug(call_ai_safely("你好"))
# logger.debug(call_ai_safely("你"*101))

# # !执行顺序
# # return None
# #   ↓
# # 执行 finally 里的 logger.debug
# #   ↓
# # 函数真正返回 None
# #   ↓
# # 外层的 logger.debug 打印 None
...
#         logger.debug(f"ID为{log_id}的日志不存在")
#         return None
#     except Exception as e:
#         logger.debug(f"查询出错：{e}")
#         return None
    
# # 测试
# logger.debug(get_log_safely(1))  # 如果id=1存在
# logger.debug(get_log_safely(999))  # 不存在，触发DoesNotExist
...
# logger.debug(type(prompt_text))  # <class 'str'>
# logger.debug(type(duration_value))  # <class 'float'>
# logger.debug(type(is_success))  # <class 'bool'>
...
#     logger.debug(f"日志ID: {log_id}")

# # 从数据库里查数据
# from ai_log.models import AICallLog
# all_logs = AICallLog.objects.all() # QuerySet，类似列表
# for log in all_logs:
#     logger.debug(f"用户问：{log.prompt}") # 打印每条记录的prompt字段
