...
class KnowledgeDocumentApiTests(TestCase):
    ...
    @patch("ai_log.views.call_ai_service")
    def test_multi_agent_ask_filters_agents_by_enabled_agents(self, mock_call_ai_service):
        mock_call_ai_service.side_effect = [
            ({
                "reply": json.dumps({
                    "selected_agents": ["memory", "retriever", "workflow", "answer"],
                    "reason": "Supervisor 原本认为需要知识库和工作流。",
                }, ensure_ascii=False),
                "prompt_tokens": 12,
                "completion_tokens": 8,
                "total_tokens": 20,
        ...
        response = self.client.post("/api/knowledge-documents/multi-agent-ask/", {
            ...
            "top_k": 3,
            "search_type": "hybrid",
            "conversation_id": "multi-agent-enabled-agents",
            "router_type": "supervisor",
            "enabled_agents": ["memory", "retriever"],
        }, format="json")
        ...
        data = response.data["data"]
        self.assertEqual(data["router_type"], "supervisor")
        self.assertEqual(data["enabled_agents"], ["memory", "retriever"])

        self.assertIn("memory", data["agents"])
        self.assertIn("retriever", data["agents"])
        self.assertIn("answer", data["agents"])
        self.assertNotIn("workflow", data["agents"])

        self.assertEqual(data["answer"], "限制 Agent 后仍然完成回答。")

        trace_id = response.headers.get("X-Trace-Id")
        self.assertTrue(trace_id)

        start_step = AiTraceStepLog.objects.get(
            trace_id=trace_id,
            user=self.user,
            step="multi_agent_start",
        )

        self.assertEqual(start_step.detail["enabled_agents"], ["memory", "retriever"])
        self.assertNotIn("workflow", start_step.detail["selected_agents"])
        ...
        self.assertIn("工作流", supervisor_step.detail["reason"])
